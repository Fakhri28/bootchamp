...
class CounterForm extends React.Component {
  handleSubmit = (form) => {
    const { updateCounter, doc } = this.props;
    updateCounter({
      variables: {
        inputCounter: {
          _id: doc._id,
  ...
  handleRemove = () => {
    const { removeCounter, doc } = this.props;
    if (confirm(`Counter will permanently DELETED!!! ARE YOU SURE???`)) {
      removeCounter({
        variables: {
          _id: doc._id,
        },
  ...
  handleStatus = (status) => {
    ...
    if (confirm(`Counter will be set to ${status}?`)) {
      actions[status]({
        variables: {
          _id: doc._id,
        },
  ...
  render() {
    ...
    return (
      ...
      >
        <SEO
          title={`${settings.name} | Counter | ${doc.name || doc._id}`}
              ...
              <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                ...
                <div className="mt-1 sm:mt-0 sm:col-span-2">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    Write a few sentences about this counter.
                  </p>
                </div>
              </div>
            </div>
