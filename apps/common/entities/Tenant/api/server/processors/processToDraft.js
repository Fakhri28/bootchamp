...
const processTenantToDraft = (tenant, party) => {
  ...
  if (tenant.status !== 'Active')
    throw new Error(`Tenant status: ${tenant.status} may not be set to Draft`);
  ...
  entityUpdate(
    ...
    'Processing processTenantToDraft',
    party,
    timestamp,
  );
  ...
  entityUpdate(
    ...
    'Set Tenant Status to Draft',
    party,
    timestamp,
  );

  return { _id: tenant._id, status: 'Draft' };
};

export default processTenantToDraft;
