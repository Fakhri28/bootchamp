...
class TenantForm extends React.Component {
  handleSubmit = (form) => {
    const { updateTenant, doc } = this.props;
    updateTenant({
      variables: {
        inputTenant: {
          _id: doc._id,
          name: form.name.value,
          tenant: parseInt(form.tenant.value, 10),
  ...
  handleRemove = () => {
    const { removeTenant, doc } = this.props;
    if (confirm(`Tenant will permanently DELETED!!! ARE YOU SURE???`)) {
      removeTenant({
        variables: {
          _id: doc._id,
        },
  ...
  handleStatus = (status) => {
    ...
    if (confirm(`Tenant will be set to ${status}?`)) {
      actions[status]({
        variables: {
          _id: doc._id,
        },
  ...
  render() {
    ...
    return (
      ...
      >
        <SEO
          title={`${settings.name} | Tenant | ${doc.name || doc._id}`}
              ...
              <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                ...
                <div className="mt-1 sm:mt-0 sm:col-span-2">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    Write a few sentences about this tenant.
                  </p>
                </div>
              </div>
            </div>
