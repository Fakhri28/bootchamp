...
class EmailSubscriberForm extends React.Component {
  handleSubmit = (form) => {
    const { updateEmailSubscriber, doc } = this.props;
  ...
  handleRemove = () => {
    const { removeEmailSubscriber, doc } = this.props;
  ...
  render() {
    ...
    return (
      ...
      >
        <SEO
          title={`${settings.name} | EmailSubscriber | ${doc.name || doc._id}`}
...
EmailSubscriberForm.defaultProps = {
  disabled: true,
};
